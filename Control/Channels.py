


#
# Sort into order of increasing period and acceptable error. Most required-precision first.
#
channels    = [
                {'ID':0, 'Duration':1, 'Period':3,   'AcceptableError':0.1} ,
                {'ID':2, 'Duration':1, 'Period':8,   'AcceptableError':0.1} ,
                {'ID':5, 'Duration':1, 'Period':8,   'AcceptableError':0.1} ,
                {'ID':1, 'Duration':1, 'Period':16,  'AcceptableError':0.1} ,
                {'ID':3, 'Duration':1, 'Period':16,  'AcceptableError':0.1} ,
                {'ID':6, 'Duration':1, 'Period':16,  'AcceptableError':0.1} ,
                {'ID':7, 'Duration':1, 'Period':16,  'AcceptableError':0.1} ,
                {'ID':4, 'Duration':1, 'Period':32,  'AcceptableError':0.1} ,
              ]


